...
class TokenGenerator
{
    ...
    {
        $tokenForLink = TokenGenerator::generateRandomBase62String(
            TokenGenerator::$tokenLength);
        $tokenHashForDatabase = TokenGenerator::calculateTokenHash($tokenForLink);
    }
    ...
    public static function isTokenValid($token)
    {
        ...
        return !is_null($token) &&
            (TokenGenerator::$tokenLength == strlen($token)) &&
            ctype_alnum($token);
    }
    ...
    public static function isTokenExpired(DateTime $creationDate)
    {
        ...
        $validFor = new DateInterval(TokenGenerator::$expiryInterval);
